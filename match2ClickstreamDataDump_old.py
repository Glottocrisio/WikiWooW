...
def preprocess_wikilinks():
    ...
    wiki_df = pd.read_csv('experiment3.txt')
    print(f"Initial wikilinks shape: {wiki_df.shape}")

    print("Cleaning entity names...")
    wiki_df['subject_clean'] = wiki_df['subject'].str.replace('http://dbpedia.org/resource/', '').replace(' ', '_')
    wiki_df['object_clean'] = wiki_df['object'].str.replace('http://dbpedia.org/resource/', '').replace(' ', '_')
...
def process_clickstream_in_memory(lookup_dict):
    ...
    print(f"Processing clickstream-enwiki-2023-11.tsv")
    print(f"Chunk size: {chunk_size:,} rows")

    try:
        # Read with minimal assumptions about the file structure
        chunks = pd.read_csv('clickstream-enwiki-2024-10.tsv.gz',
                            compression='gzip',
                            sep='\t',
